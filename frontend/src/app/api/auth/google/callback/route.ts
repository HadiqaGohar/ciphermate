...
export async function GET(request: NextRequest) {
  ...
  console.log("📥 Google OAuth Callback received:", { code: code?.substring(0, 20), error, state });
  ...
  try {
    ...
    // Store tokens securely (in cookies or secure storage)
    // Note: In production, store refresh token in httpOnly cookie
    
    // Send success response
    return sendResponse(`
      <div class="success">
        <h2>✅ Google Calendar Connected Successfully!</h2>
        <p>🎉 Your Google Calendar has been connected to CipherMate.</p>
        <p>You can now create and manage calendar events.</p>
...
function sendResponse(content: string, success: boolean, tokenData?: any) {
  const html = `
    ...
    <html>
    <head>
      <title>Google Calendar OAuth - CipherMate</title>
      <style>
    ...
    <body>
      ...
      <script>
        // Send message to parent window (CipherMate main window)
