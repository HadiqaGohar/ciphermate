...
async def google_calendar_callback(request: Request):
    """Handle Google Calendar OAuth callback - exchange token directly, no redirect loop"""
    try:
        ...
        if error:
            ...
            return RedirectResponse(
                url=f"{frontend_url}?error={error}&service=google_calendar",
                status_code=302
            )
        ...
        if not code:
            ...
            return RedirectResponse(
                url=f"{frontend_url}?error=no_code&service=google_calendar",
                status_code=302
            )
        
        # Exchange code for token DIRECTLY (no redirect to frontend)
        logger.info(f"✅ Google Calendar code received, exchanging token directly")
        ...
        if response.status_code != 200:
            ...
            return RedirectResponse(
                url=f"{frontend_url}?error=token_exchange_failed&service=google_calendar",
                status_code=302
            )
        ...
        logger.info(f"✅ Google Calendar token stored successfully for key: {user_key}")
        logger.info(f"📦 temp_tokens keys: {list(temp_tokens.keys())}")
        
        # Redirect to frontend with success message
        return RedirectResponse(
            url=f"{frontend_url}?success=true&service=google_calendar",
            status_code=302
        )
        ...
        frontend_url = getattr(settings, 'FRONTEND_URL', "https://ciphermate.vercel.app")
        return RedirectResponse(
            url=f"{frontend_url}?error=callback_failed&service=google_calendar",
            status_code=302
        )
